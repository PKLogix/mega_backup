...
def take_backups_if(freq):
    if cint(frappe.db.get_value("Mega Settings", None, "enable")):
        if frappe.db.get_value("Mega Settings", None, "frequency") == freq:
            take_backups_mega()


@frappe.whitelist()
def take_backups_mega():
    try:
        backup_to_mega()
        send_email(True, "Mega Settings")
    except Exception:
        error_message = frappe.get_traceback()
        frappe.errprint(error_message)
        send_email(False, "Mega Settings", error_message)
...
def send_email(success, service_name, error_status=None):
    if success:
        ...
        that your backup was successfully uploaded to your Mega cloud. So relax!</p> """ \
                  % frappe.db.get_value("Mega Settings", None, "company_name")

    else:
        subject = "[Warning] Backup Upload Failed"
        message = """<h3>Backup Upload Failed for %s </h3><p>Oops, your automated backup to Mega cloud failed.
        </p> <p>Error message: %s</p> <p>Please contact your system manager
        for more information.</p>""" % (frappe.db.get_value("Mega Settings", None, "company_name"), error_status)

    if not frappe.db:
        frappe.connect()

    if frappe.db.get_value("Mega Settings", None, "notification_email"):
        recipients = split_emails(frappe.db.get_value("Mega Settings", None, "notification_email"))
